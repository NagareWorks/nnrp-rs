...
- Require FFI ABI `4.3.0` and Rust artifact revision `1.0.0-preview.4.21` for provider-route orchestration, exact transport/provider identity selection, route-local security, persistent server accept tickets, complete cache-lease state, the frozen capability catalog, cancellation-safe bounded event polling, complete wire-header preservation, the closed runtime-event metadata/tail envelope, reachable transport handles, packet batches, the full-duplex browser client role runtime, explicit native transport runtime shutdown, and the release-valid explicit benchmark ABI used by downstream performance gates.
