...
- Require FFI ABI `4.4.0` and Rust artifact revision `1.0.0-preview.4.24` for provider-route orchestration, exact transport/provider identity selection, route-local security, asynchronous server admission, negotiated session identity, persistent server accept tickets, complete cache-lease and schema-registry state, cancellation-safe bounded event polling, complete wire-header preservation, the closed runtime-event metadata/tail envelope, operation-scoped trace-context correlation, reachable transport handles, packet batches, the full-duplex browser client role runtime, cancellation-safe browser ingress, explicit native transport runtime shutdown, and the release-valid explicit benchmark ABI used by downstream performance gates.
