...
- [x] Complete the route-security ABI required by host SDKs.
  - [x] Allow TCP transport security handles to configure TLS connect and listen paths.
  - [x] Preserve distinct client and server config variants under the frozen `TransportSecurityConfig` handle kind.
  - [x] Keep `route-unresolved` and `security-unsatisfied` in typed host selection diagnostics while low-level FFI reports typed endpoint and config status.
...
- [x] Preserve the singular coarse-FFI ownership boundary.
  ...
  - [x] Add architecture tests that reject per-frame calls and role adoption across multiple transport libraries.
...
- [x] Keep browser APIs aligned with native role package semantics.
  - [x] Export one browser client connection that owns the WebSocket carrier and automatic hello exchange.
  - [x] Allow one browser client connection to open and resume multiple protocol sessions.
  - [x] Keep browser session recovery tickets Rust-owned and canonically encoded.
  - [x] Close one browser session without closing sibling sessions or the shared carrier.
